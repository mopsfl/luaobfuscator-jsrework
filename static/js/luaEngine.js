...
let luaEngine = function () {
    ...
    let _init = function (frontcallback, flags = 0, force = false, content = "") {
        ...
        if (sessionId !== null) {
            ...
            _scriptInit(false, flags, content)
        }
    }
    ...
    let _scriptGet = function (callback) {
        fetch(apiUrl + "?session=" + sessionId).then(response => response.json()).then(data => {
            tokens = data.tokens
    ...
    let _parseScript = function (targetElement, tick) {
        ...
        if (tokens == null || tokens.length == 0 || (tick != -1 && (tick != lastEvent || tick != lastApiTry))) {
            return false
        }
        whitespaceCount = 0
                ...
                if (i > 100000000) {
                    ...
                    if (n.xref != 0) {
                        ...
                        if (month === 2 && day === 14) {
                            ...
                            let lightEnd = 100;
                            const max = 360;
                            var hexstr = getRandomHex((n.xref * 13) % max, max, hueStart, hueEnd, satStart, satEnd, lightStart, lightEnd);
                        } else if (month == 6) {
                            let hueStart = 0;
                            let hueEnd = 360;
                            let satStart = 100;
